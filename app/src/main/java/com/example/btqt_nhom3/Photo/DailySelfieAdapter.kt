...
class DailySelfieAdapter(
    ...
    private val onStartGlobalSelection: () -> Unit
    ...
    override fun onBindViewHolder(holder: DayVH, position: Int) {
        ...
        holder.tvDay.text = when (date) {
            ...
            todayFull -> "Hôm nay"
                ...
                runCatching { inputFormat.parse(date)?.let { outputFormat.format(it) } }
                    .getOrNull() ?: date
            }
        }

        val selfies = dailySelfies[date] ?: emptyList()
        ...
        val childAdapter = SelfieAdapter(
            selfies,
            onPhotoClick,
